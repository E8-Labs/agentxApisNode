...
async function TryAndChargePayment(
    ...
    try {
        ...
        // Add parameters to exempt from 3D Secure for automated payments
        if (isAutomatedPayment) {
            ...
            // For regular payments, include return_url for 3D Secure
            paymentIntentPayload.return_url = `${process.env.FRONTEND_URL}/payment-result`;
        }
        ...
        if (paymentIntent.status === "succeeded") {
            ...
            SendSubscriptionFailedEmail(user, plan, paymentIntent.status, paymentIntent);
            return {
                status: false,
...
export const chargeUser = async(
    ...
    req = null,
    isAutomatedPayment = false // Add this parameter
) => {
    const stripe = getStripeClient();
...
/**
 * Confirm a payment intent that requires 3D Secure authentication
 ...
 * @returns {Object} - The result of the confirmation
 */
export async function confirm3DSecurePayment(paymentIntentId, clientSecret) {
    const stripe = getStripeClient();
    ...
    try {
        ...
        return {
            status: false,
            message: `Unexpected payment intent status: ${paymentIntent.status}`,
            paymentIntent
        };
    } catch (error) {
        console.error("Error confirming 3D Secure payment:", error);
        return {
            status: false,
