    ...
    JWT.verify(req.token, process.env.SecretJwtKey, async(error, authData) => {
        if (authData) {
            ...
            try {
                ...
                await db.User.update({ lastPaymentMethodAddedAt: new Date() }, { where: { id: user.id } });
                return res.send({
                    status: added.status,
        ...
        if (authData) {
            ...
            return res.send({
                ...
                message: added.status ?
                    "Payment method set as default" : "Payment method could not be set as default",
                data: added,
            });
        } else {
            return res.send({
        ...
        if (authData) {
            ...
            try {
                if (updateFuturePlan) {
                    ...
                    await db.PlanHistory.update({ status: "upgraded" }, {
                        where: {
                            userId: user.id,
                            status: "active",
                        },
                    ...
                    if (lastPlan) {
                        ...
                        if (user.isTrial) {
                            ...
                            if (upgrade && lastPlan.status == "active") {
                                ...
                                if (
                                    ...
                                    await db.PlanHistory.update({ status: "upgraded" }, { where: { userId: user.id, status: "active" } }); //set all previous planas cancelled
                                    let planHistory = await db.PlanHistory.create({
                                        userId: user.id,
                                        type: foundPlan.type,
                                        price: foundPlan.price,
                        ...
                        await db.PlanHistory.update({ status: "upgraded" }, { where: { userId: user.id, status: "active" } }); //set all previous plans as cancelled
                        let planHistory = await db.PlanHistory.create({
                            userId: user.id,
                            type: foundPlan.type,
                            price: foundPlan.price,
                        ...
                        if (
                            ...
                            if (charge && charge.status) {
                                ...
                                return res.send({
                                    status: false,
                                    message: charge && charge.message ? charge.message : "Error upgrading ",
                                    data: null,
                                });
                            }
                        } else {
                    ...
                    return res.send({
                        status: false,
                        message: charge && charge.message ? charge.message : "Some error occurred ",
                        data: null,
                    });
                }
            } catch (error) {
    ...
    if (nextChargeDate < now) {
        ...
        if (lastPlan && lastPlan.status == "active") {
            ...
            let charge = await chargeUser(
                ...
                true,
                null,
                true // Set isAutomatedPayment to true for subscription renewals
            );
        ...
        let charge = await chargeUser(
            ...
            false,
            null,
            true // Set isAutomatedPayment to true for low balance charges
        );
        // user = await db.User.findByPk(user.id);
        // console.log("Charge ", charge);
        if (charge && charge.status) {
...
/**
 * Handle 3D Secure authentication confirmation
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 */
export async function handle3DSecureConfirmation(req, res) {
    JWT.verify(req.token, process.env.SecretJwtKey, async(error, authData) => {
        if (error) {
        ...
        try {
            const result = await confirm3DSecurePayment(paymentIntentId, clientSecret);
            ...
            console.error("Error in 3D Secure confirmation:", error);
